...
begin
    ...
    scatter!(ax3d, x_teach[1:ind, 1], x_teach[1:ind, 2]; color = :red, markersize = 10)
    # scatter!(ax3d, x_teach[1:ind, 1], x_teach[1:ind, 2], x_teach[1:ind, 3]; color = :red, markersize = 10)
    display(fig)
...
begin
    ind = 500    
    fig2 = Figure(resolution = (1200, 900), padding = 40)

    # XY Plane
    ax_xy = Axis(fig2[1, 1]; title = "XY Plane", xlabel = "X [mm]", ylabel = "Y [mm]", aspect = 1)
    scatter!(ax_xy, x_teach[1:ind, 1], x_teach[1:ind, 2]; color = :red, markersize = 10)

    # XZ Plane
    ax_xz = Axis(fig2[1, 2]; title = "XZ Plane", xlabel = "X [mm]", ylabel = "Z [mm]", aspect = 1)
    scatter!(ax_xz, x_teach[1:ind, 1], x_teach[1:ind, 3]; color = :red, markersize = 10)

    # YZ Plane
    ax_yz = Axis(fig2[1, 3]; title = "YZ Plane", xlabel = "Y [mm]", ylabel = "Z [mm]", aspect = 1)
    scatter!(ax_yz, x_teach[1:ind, 2], x_teach[1:ind, 3]; color = :red, markersize = 10)
...
serialize("TestRobotarm_Neural/results/trained_network_Flux" * string(round(losses[end], digits = 5)) * ".jls", model)
